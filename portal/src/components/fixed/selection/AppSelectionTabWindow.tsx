...
export const AppSelectionTabWindow = ({ positionId }: AppSelectionTabWindowProps) => {
    ...
    const setPendingPublicationList = useAppSelectionStore((state) => state.setPendingPublicationList)

    const setFulfilledPublicationList = useAppSelectionStore((state) => state.setFulfilledPublicationList)
    ...
    return (<>
        <div className="w-full bg-inherit rounded px-5 h-[28rem] overflow-y-auto">
            {tab === 1 && <AllSelectionPublicationListTab publications={pendingPublicationList} tab={tab} />}
            {tab === 2 && <AllSelectionPublicationListTab publications={fulfilledPublicationList} tab={tab} />}
        </div>
    </>)
