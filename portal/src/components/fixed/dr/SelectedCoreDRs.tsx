...
export const SelectedCoreDRs = (): JSX.Element => {
  ...
  const handleRemove = (positionIndexToRemove: number, odrIdToRemove: string, ogdrId: string) => {
    ...
    let sortedNewPool = updatedAllDRPool.sort((a: DutyResponsibility, b: DutyResponsibility) => a.description!.localeCompare(b.description!))
  ...
  const handleEditToggle = (odrId: string, onEdit: boolean, sequenceNo: number) => {
    ...
    <>
      <Table
        ...
        tableBody={
          <>
            {/* <SelectedCoreDRs /> */}
            {selectedDRCs && selectedDRCs.core.map((dr: DutyResponsibility, index: number) => {
              return (
                <tr key={index} className="font-light text-gray-600 peer hover:text-white hover:bg-gray-400 ">
                  <td className="flex items-start justify-between py-1 text-sm ">
                    ...
                    <div className="w-[10%] flex items-center justify-center gap-2">
                      {dr.onEdit ? (
                        <>
                          <HiLockOpen
                            ...
                            fill="#fc0303"
                            onClick={() => handleEditToggle(dr.odrId, dr.onEdit!, dr.sequenceNo!)}
                          />
                        </>
                      ) : (
                        <>
                      ...
                      <HiX
                        ...
                        fill="white"
                        size={30}
                        onClick={() => handleRemove(index, dr.odrId, dr.ogdrId!)}
                      />
                    </div>
                  </td>
                </tr>
