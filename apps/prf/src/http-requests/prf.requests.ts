...
export const approvePrf = async (prfId: string, status: string, employeeId: string, remarks: string) => {
  const { data } = await axios.patch(`${url}/prf-trail/${prfId}`, { status, employeeId, remarks });

  return data;
};

export const savePrf = async (prfData: CreatedPrf) => await post(`${url}/prf`, prfData);
