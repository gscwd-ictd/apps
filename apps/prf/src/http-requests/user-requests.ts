...
export async function userSignin(email: string, password: string, callBack: (result: string) => void) {
  // attempt to sign in
  const { error, result } = await post(`${baseUrl}/users/web/signin`, { email, password });
...
export async function verifyPendingUser(userId: string, context?: GetServerSidePropsContext) {
  const { data } = await axios.get(`${baseUrl}/pending-users/verify/${userId}`, {
...
export async function getUserDetails(userId: string, context?: GetServerSidePropsContext) {
...
export async function verifySignupToken(token: string) {
  ...
  const { data } = await axios.get(`${baseUrl}/users/web/verify-signup/${token}`);
