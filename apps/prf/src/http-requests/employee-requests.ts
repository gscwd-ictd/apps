...
export const createEmployee = async (pendingUser: PendingUser) => {
  try {
    return await axios.post(`${baseUrl}/employees`, pendingUser, { withCredentials: true });
  } catch (error) {
    console.log(error, 'employee-requests: error coming from create employee profile');
  }
};
