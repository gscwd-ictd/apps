...
import { postEmpMonitoringWithFile } from 'apps/employee-monitoring/src/utils/helper/employee-monitoring-axios-helper';
...
const AddAnnouncementModal: FunctionComponent<AddModalProps> = ({ modalState, setModalState, closeModalAction }) => {
  ...
  const handlePostResult = async (data: FormPostAnnouncement) => {
    ...
    const { error, result } = await postEmpMonitoringWithFile('/events-announcements', formData);
