...
import { EmployeeAsOption, EmployeeProfile } from 'libs/utils/src/lib/types/employee.type';
...
const AddImmediateSupervisorModal: FunctionComponent<AddModalProps> = ({
  ...
  const [selectedEmployee, setSelectedEmployee] = useState<EmployeeAsOption>({} as EmployeeAsOption);
    ...
    employeeQuery === ''
      ...
      : // ? EmployeeAsOptions
        EmployeeAsOptions.filter((employee) =>
          // : EmployeeAsOptions.filter((employee) =>
          employee.fullName.toLowerCase().replace(/\s+/g, '').includes(employeeQuery.toLowerCase().replace(/\s+/g, ''))
        );
  ...
  const fetchEmployeeList = async () => {
    ...
    if (error) {
      ...
      const employeesDetails: Array<EmployeeAsOption> = result.map((employeeDetails: EmployeeProfile) => {
  ...
  return (
    <>
      {/* Error Notifications */}
      {/* {!isEmpty(ErrorEAO) ? <ToastNotification toastType="error" notifMessage={ErrorEAO} /> : null} */}
            ...
            <form onSubmit={handleSubmit(onSubmit)} id="addImmediateSupervisorForm">
              ...
              <div className="grid md:grid-cols-2 md:gap-6">
                ...
                {/* <div className="z-20 mb-6">
                    ...
                    <div className="relative mt-1">
                      <div className="relative w-full overflow-hidden text-left bg-white rounded-lg cursor-default focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
                        ...
                        bg-gray-50 border ${
                          ...
                          displayValue={(selectedEmployee: EmployeeAsOption) => selectedEmployee.fullName}
                          onChange={(event) => {
                      ...
                      >
                        ...
                        max-h-[14rem]"
                        >
                          {filteredEmployee.length === 0 && employeeQuery !== '' ? (
                            ...
                            filteredEmployee.map((employee) => (
                              ...
                              >
                                {({ selected, active }) => (
                                  <>
                                    <span className={`block truncate ${selected ? 'text-sm' : 'text-xs'}`}>
                                      {employee.fullName}
                                    </span>
                  ...
                  {errors.employee ? <div className="mt-1 text-xs text-red-400">{errors.employee?.message}</div> : null}
                </div> */}
