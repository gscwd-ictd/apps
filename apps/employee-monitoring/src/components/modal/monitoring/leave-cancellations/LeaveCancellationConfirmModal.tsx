...
type LeaveCancellationConfirmModalProps = {
  modalState: boolean;
...
const LeaveCancellationConfirmModal: FunctionComponent<LeaveCancellationConfirmModalProps> = ({
  modalState,
  closeModalAction,
  setModalState,
  action,
}) => {
  const { SetLeaveCancellationConfirmAction } = useLeaveApplicationStore((state) => ({
    SetLeaveCancellationConfirmAction: state.setLeaveCancellationConfirmAction,
  }));

  // submit
  const onSubmit = (confirmAction: LeaveConfirmAction) => {
    SetLeaveCancellationConfirmAction(confirmAction);
    closeModalAction();
  };
  ...
  return (
    <>
      <Modal open={modalState} setOpen={setModalState} size="xs" steady>
        ...
        <Modal.Body>
          <div className="px-5 text-gray-800">Do you want to {action} this leave cancellation request?</div>
        </Modal.Body>
        <Modal.Footer>
...
export default LeaveCancellationConfirmModal;
