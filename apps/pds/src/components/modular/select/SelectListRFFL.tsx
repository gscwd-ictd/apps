      ...
      >
        <div className="relative">
          <select
            ...
            // placeholder={defaultOption}
            disabled={muted}
            className={`${muted && 'cursor-not-allowed'} w-full cursor-pointer appearance-none px-[1.12rem] pt-4 pb-4  font-normal text-gray-600 
            ${isError ? background['danger'] : background[variant]} ${isError ? focusWithin['danger'] : focusWithin[variant]} ${
       ...
       text-slate-600   ${className}`}
          ...
          <label
            htmlFor={id}
            className={`peer-focus:-pt-1 -pt-1 absolute -top-6 left-0  mx-4 mt-4
          h-fit cursor-text  px-1 text-xs font-normal text-gray-700 transition-all peer-placeholder-shown:-inset-y-[0.85rem] peer-placeholder-shown:left-0 peer-placeholder-shown:text-base  ${
            muted ? 'peer-placeholder-shown:text-gray-400' : 'peer-placeholder-shown:text-gray-600'
          } bg-white peer-placeholder-shown:px-1 peer-focus:-top-11 peer-focus:left-0  peer-focus:h-fit peer-focus:bg-white peer-focus:px-1
