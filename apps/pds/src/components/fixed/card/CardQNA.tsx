...
export const CardQuestion = ({ cols = 4, children, mainQuestion }: QuestionCardProps): JSX.Element => {
  return (
    <>
      <div
        className={`grid grid-cols-${cols} mb-0 rounded-md border  bg-white lg:px-8 shadow-sm shadow-slate-200 hover:bg-indigo-100 peer-hover:bg-indigo-100 `}
      >
...
export const LabelQNA = ({
  ...
  return (
    <>
      <div
        className={`flex grid-cols-${cols} mt-5 min-w-fit  hover:-mx-2 text-slate-700 hover:text-black  hover:transition-all`}
      >
        <p className={`col-span-1 mt-0 w-[50%] pr-10 `}>{question}</p>
