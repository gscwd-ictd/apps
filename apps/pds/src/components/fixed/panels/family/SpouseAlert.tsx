...
export const SpouseAlert = ({ setInitialValues }: SpouseAlertProps): JSX.Element => {
  ...
  const setAllowSpouseSave = useUpdatePdsStore((state) => state.setAllowSpouseSave);
  ...
  const updateSection = async (): Promise<Actions> => {
    // await...
    try {
      await axios.put(`${process.env.NEXT_PUBLIC_PORTAL_URL}/pds/family/spouse/${employeeDetails.user._id}`, pds.spouse);
  ...
  return (
    <>
      <Alert open={alertUpdateIsOpen} setOpen={setAlertUpdateIsOpen}>
        <Alert.Description>
          <AlertDesc>Do you want to update your Spouse's Information? This action is irreversible.</AlertDesc>
        </Alert.Description>
      ...
      <Alert open={alertCancelIsOpen} setOpen={setAlertCancelIsOpen}>
        <Alert.Description>
          <AlertDesc>Are you sure you want to cancel the changes that you have made to your Spouse's Information?</AlertDesc>
        </Alert.Description>
        <Alert.Footer alignEnd>
          <div className="w-full rounded border border-gray-300">
      ...
      {hasPds && (
        <div className="h-[1.6rem] w-auto">
          ...
          {!spouseOnEdit && (
            <>
              <Button onClick={() => setSpouseOnEdit!(true)} btnLabel="" variant="light" type="button" className="ring-0 hover:bg-white focus:ring-0">
                <div className="flex items-center text-gray-400 hover:text-gray-600">
                  <HiPencil size={20} />
                  <span>Edit</span>
                </div>
