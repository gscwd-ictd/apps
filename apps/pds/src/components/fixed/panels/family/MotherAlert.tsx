...
export const MotherAlert = ({ setInitialValues }: MotherAlertProps): JSX.Element => {
  ...
  const setAllowMotherSave = useUpdatePdsStore((state) => state.setAllowMotherSave);
  ...
  const updateSection = async () => {
    const { fatherFirstName, fatherLastName, fatherMiddleName, fatherNameExtension, ...rest } = pds.parents;

    try {
      await axios.put(`${process.env.NEXT_PUBLIC_PORTAL_URL}/pds/family/parents/${employeeDetails.user._id}/mother`, rest);
      setInitialPdsState({
        ...initialPdsState,
        parents: {
  ...
  useEffect(() => {
    if (
      motherOnEdit &&
      (isEmpty(parents.motherLastName) || isEmpty(parents.motherFirstName) || isEmpty(parents.motherMiddleName) || isEmpty(parents.motherMaidenName))
    ) {
  ...
  return (
    <>
      <Alert open={alertUpdateIsOpen} setOpen={setAlertUpdateIsOpen}>
        <Alert.Description>
          <AlertDesc>Do you want to update your Mother&apos;s Maiden Name? This action is irreversible.</AlertDesc>
        </Alert.Description>
      ...
      <Alert open={alertCancelIsOpen} setOpen={setAlertCancelIsOpen}>
        <AlertDesc>Are you sure you want to cancel the changes that you have made to your Mother&apos;s Maiden Name?</AlertDesc>
        <Alert.Footer alignEnd>
          <div className="w-full border border-gray-300 rounded">
      ...
      {hasPds && (
        <div className="h-[1.6rem] w-auto">
          ...
          {!motherOnEdit && (
            <>
              <Button onClick={() => setMotherOnEdit!(true)} btnLabel="" variant="light" type="button" className="ring-0 hover:bg-white focus:ring-0">
                <div className="flex items-center text-gray-400 hover:text-gray-600">
                  <HiPencil size={20} />
                  <span>Edit</span>
                </div>
