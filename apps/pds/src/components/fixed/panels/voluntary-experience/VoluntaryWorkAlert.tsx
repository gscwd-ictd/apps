...
export const VoluntaryWorkAlert = ({ setInitialValues }: VoluntaryWorkAlertProps): JSX.Element => {
  ...
  const setVoluntaryWorkOnEdit = usePdsStore((state) => state.setVoluntaryWorkOnEdit);
  ...
  const setDeletedVolWork = useUpdatePdsStore((state) => state.setDeletedVolWork);
  ...
  const updateSection = async () => {
    ...
    try {
      const { data } = await axios.post(`${process.env.NEXT_PUBLIC_PORTAL_URL}/pds/voluntary-work/${employeeDetails.user._id}`, {
        add: allVolWorks.add,
  ...
  return (
    <>
      <Alert open={alertUpdateIsOpen} setOpen={setAlertUpdateIsOpen}>
        <Alert.Description>
          <AlertDesc>Do you want to update your Voluntary Work Experience? This action is irreversible.</AlertDesc>
        </Alert.Description>
      ...
      <Alert open={alertCancelIsOpen} setOpen={setAlertCancelIsOpen}>
        <Alert.Description>
          <AlertDesc>Are you sure you want to cancel the changes that you have made to your Voluntary Work Experience?</AlertDesc>
        </Alert.Description>
        <Alert.Footer alignEnd>
          <div className="w-full rounded border border-gray-300">
      ...
      {hasPds && (
        <div className="h-[1.6rem] w-auto">
          ...
          {!voluntaryWorkOnEdit && (
            <>
              <Button
                onClick={() => setVoluntaryWorkOnEdit!(true)}
                btnLabel=""
                variant="light"
                type="button"
                className="ring-0 hover:bg-white focus:ring-0"
