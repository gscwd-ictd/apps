...
export const SecondaryAlert = ({ setInitialValues }: SecondaryAlertProps): JSX.Element => {
  ...
  const allowSecondarySave = useUpdatePdsStore((state) => state.allowSecondarySave);
  const setAllowSecondarySave = useUpdatePdsStore((state) => state.setAllowSecondarySave);
  ...
  const updateSection = async () => {
    ...
    try {
      await axios.put(`${process.env.NEXT_PUBLIC_PORTAL_URL}/pds/education/secondary/${employeeDetails.user._id}`, rest);
  ...
  return (
    <>
      <Alert open={alertUpdateIsOpen} setOpen={setAlertUpdateIsOpen}>
        <Alert.Description>
          <AlertDesc>Do you want to update your Secondary Education? This action is irreversible.</AlertDesc>
        </Alert.Description>
      ...
      <Alert open={alertCancelIsOpen} setOpen={setAlertCancelIsOpen}>
        <Alert.Description>
          <AlertDesc>Are you sure you want to cancel the changes that you have made to your Secondary Education?</AlertDesc>
        </Alert.Description>
        <Alert.Footer alignEnd>
          <div className="w-full border border-gray-300 rounded">
