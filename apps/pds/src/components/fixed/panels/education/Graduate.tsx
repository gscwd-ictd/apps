...
export const Graduate = (): JSX.Element => {
  ...
  return (
    <>
      ...
      >
        <>
          <div className={`flex flex-col items-end justify-end ${graduateOnEdit ? 'visible' : !hasPds ? 'visible lg:-mt-6 lg:pb-6' : 'hidden'}`}>
            <Button btnLabel="Add Graduate Studies" type="button" variant="theme" onClick={openModal} className="sm:w-full lg:w-56" />
          </div>
                              ...
                              <>
                                {!isEmpty(course._id) ? (
                                  <div className="flex justify-center gap-4">
                                    {allowEditGraduate ? (
                                      <div className="w-8">
                                        <EditButton action={() => onEdit(course, courseIdx)} type="button" disabled={graduateOnEdit ? false : true} />
                                      </div>
                                    ) : null}
                                    {allowDeleteGraduate ? (
                                      <div className="w-8">
                                        <DeleteButton
                                          muted={hasPds && graduateOnEdit ? false : hasPds && !graduateOnEdit ? true : !hasPds && false}
                                          action={() => openRemoveActionModal(courseIdx, course)}
                                        />
                                      </div>
                                    ) : null}
                                    {!allowEditGraduate && !allowDeleteGraduate ? <div className="flex justify-center w-full">-</div> : null}
                                  </div>
                                ) : isEmpty(course._id) ? (
                                  <div className="flex justify-center gap-4">
                                    <div className="w-8">
