...
const schema = yup.object().shape({
  email: yup.string().required('Email Address is not provided').label('Email Address'),
  password: yup.string().required('Password is not provided').label('Password'),
});

export default function Login(): JSX.Element {
  // initialize an Error state
  const [error, setError] = useState<Error>({ status: false, message: null, animate: false });
  ...
  useEffect(() => {
    ...
    if (router.query.error === 'timeout' && (email || password)) {
      ...
      setError({ status: true, message: 'Your connection with the server has timed out', animate: true });
   ...
   */
  const login: SubmitHandler<LoginFormInput> = async (credentials: LoginFormInput) => {
    try {
      ...
      const { error, result } = await postData('http://192.168.137.249:4002/user/signin', { email, password });

      // check if the result return an error, go to dashboard otherwise
      result && router.push(`/dashboard/${result._id}`);
    } catch (error) {
      setIsLoading(false);
      setError({ status: true, message: 'Cannot connect to server.', animate: true });
    }
  };
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="grid-cols-3 flex-col justify-center gap-4 overflow-x-visible pt-24 sm:flex lg:grid ">
          <div className="col-span-2">
            <div className="flex flex-col items-center">
              <h1 className="-mt-9 select-none border-dashed border-indigo-500 bg-transparent px-5 py-2 text-5xl font-medium text-indigo-700">
                Personal Data Sheet
              </h1>
          ...
          <div className="col-span-1">
            <div className="flex min-w-max flex-col items-center ">
              <main className="rounded-lg border border-slate-100 bg-white  p-5 sm:mx-[3%] lg:w-[30rem]">
                <div>
                  <h2 className="select-none text-2xl font-medium text-slate-700">Sign in</h2>
                  <h2 className="mb-8 select-none text-xs font-normal text-slate-500"> Welcome back! Please enter your credentials below.</h2>
                </div>
                ...
                {error.status && (
                  <section className="mb-3">
                    <div
                      ...
                      className={`${error.animate ? 'animate-shake' : null} flex items-center gap-1 border-l-4 border-rose-300 bg-red-100 p-3`}
                    >
                ...
                <form onSubmit={handleSubmit(login)} id="login" className="max-w-md">
                  <div className="flex flex-col justify-center">
                    ...
                    <div className="mt-5 flex items-center justify-between">
                      <div>
                        <input
                          ...
                          className="cursor-pointer rounded-sm checked:bg-indigo-500 checked:hover:bg-indigo-600 focus:ring-transparent focus:checked:bg-indigo-600"
                        />
                        <label htmlFor="checkboxRemember" className="ml-2 cursor-pointer text-xs font-normal text-slate-600">
                          Remember me
                        </label>
                      </div>
                    ...
                    <div>
                      ...
                      >
                        <span className="flex items-center justify-center gap-2">
                          <p className="text-sm font-medium uppercase">{isLoading ? 'Processing' : 'Login'}</p>
                    ...
                    {/* Etc */}
                    <div className="mt-10 block text-center text-xs text-slate-400">Copyright © 2022. All rights reserved.</div>
                    <div className="block text-center text-xs text-slate-400">General Santos City Water District</div>
                  </div>
                </form>
              </main>
            </div>
