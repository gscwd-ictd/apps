...
export const useApplicantStore = create<ApplicantState>((set, get) => ({
  ...
  submission: {
    hasSubmitted: false,
    hasRelevantWorkExperience: { withRelevantExperience: false },
  },
  setApplicant: (applicant: ApplicantFormData) =>
    set((state) => ({ ...state, applicant })),
  setPage: (page: number) => set((state) => ({ ...state, page })),
  setPositionTitle: (positionTitle: string) =>
    set((state) => ({ ...state, positionTitle })),
  setIsExistingApplicant: (isExistingApplicant: boolean) =>
    set((state) => ({ ...state, isExistingApplicant })),
