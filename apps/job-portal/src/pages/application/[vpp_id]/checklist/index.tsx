...
export default function Checklist({ vppId, positionTitle }: ChecklistProps) {
  ...
  const onClickPds = () => {
    router.push(`${process.env.NEXT_PUBLIC_JOB_PORTAL}/application/${vppId}/pds`);
  };
  ...
  // this function determines the current step and what actions they can do
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      await axios.get(`${process.env.NEXT_PUBLIC_HRIS_DOMAIN}/vacant-position-postings/publications/portal/${vppId}`, {
      ...
      if (postWorkExperienceSheet.status === 200 || postWorkExperienceSheet.status === 201) {
        // localStorage.clear()
        setSubmission({
          ...submission,
          hasSubmitted: true,
  ...
  useEffect(() => {
    ...
    else if (progress.firstStep === true && !hasPds) setProgress({ ...progress, secondStep: false });
  }, [progress.firstStep, hasPds]);

  useEffect(() => {
    if (progress.secondStep === true) setProgress({ ...progress, thirdStep: false });
  }, [progress.secondStep]);

  useEffect(() => {
    if (
      (progress.thirdStep === false && (workExperiencesSheet.length > 0 || noWorkExperience === true)) ||
      submission.hasSubmitted === true
    ) {
      setProgress({ ...progress, thirdStep: true });
    }
  }, [progress.thirdStep, workExperiencesSheet.length, submission.hasSubmitted]);
  ...
  useEffect(() => {
    ...
    // applicant has submitted
      ...
      setSubmission({
        ...
        hasSubmitted: true,
        hasRelevantWorkExperience: {
          withRelevantExperience: hasWE.hasRelevantWorkExperience.withRelevantWorkExperience ? true : false,
        },
      });
    }
  }, [hasWE]);
        ...
        <Alert open={alertSubmitIsOpen} setOpen={setAlertSubmitIsOpen}>
          <Alert.Description>
            <div className="flex flex-col justify-center w-full h-full">
              ...
              <span className="text-md">
                ...
                <span className="font-bold text-black">{positionTitle}</span> position:
                <br />
              </span>
          ...
          <Alert.Footer alignEnd>
            <div className="flex gap-2">
              <Button onClick={() => setAlertSubmitIsOpen(false)} className="w-[6rem]">
                No
              </Button>
        ...
        <Alert open={alertSuccessIsOpen} setOpen={setAlertSuccessIsOpen}>
          <Alert.Description>
            <div className="flex h-[3rem] w-full ">
              ...
              <div className="flex w-[85%] items-center text-3xl font-medium text-green-500">Success</div>
            </div>
            <div className="flex flex-col justify-center w-full h-full">
              <span className="text-lg font-light">Your application has been submitted!</span>
        ...
        <Alert open={alertErrorIsOpen} setOpen={setAlertErrorIsOpen}>
          <Alert.Description>
            <div className="flex h-[3rem] w-full ">
              ...
              <div className="flex w-[85%] items-center text-3xl font-medium text-red-600">Error</div>
            </div>
            <div className="flex flex-col justify-center w-full h-full">
              <span className="text-lg font-light">An error has been encountered</span>
              <span className="text-lg font-light">Please try again in a few seconds</span>
            </div>
          </Alert.Description>
          <Alert.Footer alignEnd>
            <div className="w-[6rem]">
                    ...
                    <table className="">
                      ...
                      <tbody className="divide-y-4 divide-dotted">
                        ...
                        <tr>
                          ...
                          <td
                            className={`w-[20%] py-5 text-center text-lg ${
                              progress.firstStep === true ? 'text-green-500' : 'text-rose-500'
                            }`}
                          >
                            <div className="flex justify-center w-full h-full">
                              {progress.firstStep === true && submission.hasSubmitted === false ? (
                                <>
                                  <div className="w-[6rem] rounded bg-green-500 py-1 px-3 text-white">Done</div>
                                </>
                              ) : progress.firstStep === true && submission.hasSubmitted === true ? (
                                <div className="w-[6rem] rounded bg-green-500 py-2 px-3 text-sm text-white">
                                  Submitted
                                </div>
                              ) : (
                                <div className="w-[6rem] rounded bg-red-500 py-1 px-3 text-white">Pending</div>
                              )}
                            </div>
                          </td>
                        </tr>
                        ...
                        <tr>
                          ...
                          <td className="w-[20%] py-5 text-center uppercase text-indigo-500">
                            <div className="flex justify-center w-full h-full">
                              ...
                              submission.hasSubmitted === false ? (
                                <>
                                  <div className="w-[6rem]">
                                    <Button onClick={onClickPds} disabled={submission.hasSubmitted}>
                                      <div className="font-medium text-white text-md">Update</div>
                                    </Button>
                                  </div>
                                </>
                              ) : progress.secondStep === true || submission.hasSubmitted === true ? (
                                <span className="text-black">-</span>
                              ) : progress.firstStep === true && !hasPds ? (
                                <>
                                  <div className="w-[6rem]">
                                    <Button onClick={onClickPds} type="button" disabled={submission.hasSubmitted}>
                                      <div className="font-medium text-white text-md">Create</div>
                                    </Button>
                                  </div>
                                </>
                          ...
                          <td
                            className={`w-[20%]  py-5 text-center text-lg ${
                              progress.secondStep === true ? 'text-green-500' : 'text-rose-500'
                            }`}
                          >
                            {progress.secondStep === true && submission.hasSubmitted === false ? (
                              <>
                                <div className="w-[6rem] rounded bg-green-500 py-1 px-3 text-white">Done</div>
                              </>
                            ) : submission.hasSubmitted === true ? (
                        ...
                        <tr>
                          ...
                          <td className="w-[20%] py-5 text-center  ">
                            <div className="flex justify-center w-full h-full">
                              ...
                              submission.hasSubmitted === false ? (
                                <>
                                  <div className="w-[6rem]">
                                    <Button onClick={onClickWorkExpSheet} disabled={submission.hasSubmitted}>
                                      <div className="font-medium text-white text-md">Update</div>
                                    </Button>
                                  </div>
                                </>
                          ...
                          <td className="w-[15%] py-5 text-center ">
                            {progress.thirdStep === true && noWorkExperience === false ? (
                              <>
                                <div className="flex justify-center w-full">
                                  <a
                                    href={`${process.env.NEXT_PUBLIC_JOB_PORTAL}/application/${vppId}/${externalApplicantId}/work-experience-sheet/pdf`}
                          ...
                          <td
                            className={` w-[20%]  py-5 text-center text-lg ${
                              progress.thirdStep === true ? 'text-green-500' : 'text-rose-500'
                            }`}
                          >
                            {progress.thirdStep === true ? (
                              <>
                                {noWorkExperience && submission.hasSubmitted === false ? (
                                  <div
                                    className={`bg-green-500' min-w-[6rem] rounded bg-green-500 py-1 px-3 text-white`}
                                  >
                                    <div className="flex gap-2">
                                      <span>Done</span>
                                      <MyPopover text="No relevant experience included" textClassName="text-black" />
                                    </div>
                                  </div>
                                ) : noWorkExperience && submission.hasSubmitted === true ? (
                                  <div
                                ...
                                ) : noWorkExperience === false && submission.hasSubmitted === true ? (
                                  <div
                                    className={`bg-green-500' min-w-[6rem] rounded bg-green-500 py-2 px-3 text-sm text-white`}
                                  >
                                    <div>
                                ...
                                ) : noWorkExperience === false && submission.hasSubmitted === false ? (
                                  <div
                                    className={`bg-green-500' min-w-[6rem] rounded bg-green-500 py-1 px-3 text-white`}
                                  >
                                    <span>Done</span>
              ...
              <div className="mx-2 flex flex-col gap-2">
                ...
                <div className="flex w-full gap-2 justify-between border border-gray-300 rounded px-2 py-10 bg-white">
                  ...
                  <section className="text-xs w-[30%] flex justify-center items-center">
                    {!progress.firstStep ? (
                      <div className="w-[4rem] rounded bg-red-500 py-1 px-3 text-white flex justify-center">
                        Pending
                      </div>
                    ) : (
                      <div className={`bg-green-500 w-[4rem] rounded py-1 px-3 text-white flex justify-center`}>
                        <span>Done</span>
                      </div>
                    )}
                  </section>
                  <section className="w-[10%]">
                    <VerticalDropdown items={[]} />
                  </section>
                </div>
                ...
                <div className="flex w-full gap-2 justify-between py-10 px-2 border border-gray-300 rounded bg-white">
                  ...
                  <section className="w-[10%]">
                    <VerticalDropdown items={getSecondActions()} />
                  </section>
                </div>
