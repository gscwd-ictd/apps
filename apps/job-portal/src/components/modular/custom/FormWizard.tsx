...
export const FormWizard = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement> & FormWizardProps>(
  ({ selectedTab, setSelectedTab, onClick, tabsLength, tabs, className, ...props }, ref) => {
    return (
      <>
        <div {...props} className={`${className} flex`} ref={ref}>
          ...
          <main className={`flex flex-col `}>
            <div className={`flex items-center sm:w-[5rem] md:w-[5rem] lg:w-[8rem]`}>
              <section
                ...
                }   border-indigo-400 ${startingIndex ? `invisible` : `visible`} `}
              />
              <button
                tabIndex={-1}
              ...
              <section
                className={`z-10 h-[0.25rem] w-[80%]  ${isDone ? `bg-green-600` : `bg-gray-300`} ${
