import { MyComponentSize } from 'apps/job-portal/utils/types/components/attributes';
import { MyTextFieldType } from 'apps/job-portal/utils/types/components/textfield';
...
interface MyInputReactFormProps
...
export const border = {
  ...
  simple: 'rounded-xl border border-gray-300 ',
};

export const focus = {
  default: 'focus:ring focus:border focus:border-gray-500 focus:ring-gray-100',
  primary:
    'focus:ring focus:border focus:border-gray-700 focus:ring-indigo-100',
  secondary:
    'focus:ring focus:border  focus:border-emerald-600 focus:ring-green-100',
  warning:
    'focus:ring focus:border focus:border-orange-500 focus:ring-orange-100',
  danger:
    'focus:ring focus:border focus:border-rose-600 border focus:ring-rose-100',
...
export const focusWithin = {
  ...
  light:
    'focus-within:ring-1 focus-within:border focus-within:border-indigo-500 focus-within:ring-indigo-100',
  simple: '',
};
  ...
  return (
    <>
      {withLabel ? (
        <label
          htmlFor={id}
          className="flex pl-1 pr-4 font-sans text-sm tracking-wide text-gray-600 select-none"
        >
          {label}
          {labelIsRequired ? (
            <>
      ...
      <input
        ...
        className={`${className} ${fluid ? 'w-full' : ''}  ${
          isError
            ? `${focus['danger']} ${border['danger']} ${background['danger']}`
            : `${focus[variant]} ${border[variant]} ${background[variant]} `
        } 
        ...
        ${
          muted
            ? ` cursor-not-allowed bg-slate-100 focus:border-slate-200 focus:ring-0`
            : `${background[variant]}`
        } ${
          size[inputSize]
