  ...
  return (
    <>
      {withLabel ? (
        <label
          htmlFor={id}
          className="flex px-4 font-medium text-gray-600 select-none"
        >
          {labelIsRequired ? (
            <>
      ...
      <input
        ...
        placeholder={placeholder}
        {...props}
        className={`${className} ${fluid ? 'w-full' : ''} ${border[variant]} ${
          focus[variant]
        } ${focusWithin[variant]}  ${
          muted
          ...
          size[inputSize]
        } border-gray-300 text-gray-900 placeholder-gray-400 transition-colors focus:outline-none`}
      />
    </>
  );
};
