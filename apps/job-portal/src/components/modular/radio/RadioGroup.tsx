...
const RadioGroup: React.FC<MyRadioGroupProps> = ({
  ...
  return (
    <>
      <div
        className={`${className} ${isFlex ? 'flex' : ''} ${isError ? background['danger'] : background[variant]}
        ${isError ? border['danger'] : border[variant]}
         select-none gap-4  ${focusOutline && 'focus-within:border focus-within:border-indigo-600'} `}
        id={groupName}
        name={groupName}
        disabled={muted}
        {...props}
