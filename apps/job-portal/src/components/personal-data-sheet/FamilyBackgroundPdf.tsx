...
import { Parent, Spouse } from 'apps/job-portal/utils/types/data/family.type';
import { Child } from 'apps/job-portal/utils/types/data/pds.type';
...
export const FamilyBackgroundPdf = ({ formatDate, parents, spouse, child }: FamilyBackgroundPdfProps): JSX.Element => {
  const [emptyChildRows, setEmptyChildRows] = useState(12);

  const renderChildrenRows = () => {
    const content = child.slice(0, 12).map((child, index) => (
  ...
  const renderEmptyChildrenRows = () => {
    const content = [];
    const rowToRender = emptyChildRows - child.length;
    ...
    <View>
      ...
      <View style={[styles.borderTop, { flexDirection: 'row', alignItems: 'stretch' }]}>
        ...
        <View style={[styles.w42_9]}>
          ...
          {child.length < 12 ? <>{renderEmptyChildrenRows()}</> : null}
