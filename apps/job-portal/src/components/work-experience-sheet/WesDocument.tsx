...
import { Data } from '../../types/data/wes.type'
...
export const WesDocument = ({ formatDate, workExperiencesSheet, isSubmitted, applicant }: Data): JSX.Element => {
        ...
        <View style={[styles.workExperiences]} key={index} wrap={false}>
          <View style={[styles.workExperienceContainer]}>
            <Text style={[styles.mainListStyle]}>
              • Duration: {formatDate(experience.from)} - {experience.to != null ? formatDate(experience.to) : 'PRESENT'}
            </Text>
            <Text style={[styles.mainListStyle]}>• Position: {experience.positionTitle}</Text>
            <Text style={[styles.mainListStyle]}>• Name of Office/Unit: {experience.nameOfOffice}</Text>
            <Text style={[styles.mainListStyle]}>• Immediate Supervisor: {experience.immediateSupervisor}</Text>
            <Text style={[styles.mainListStyle]}>• Name of Agency/Organization and Location: {experience.companyName}</Text>
  ...
  const renderAccomplishments = (accomplishmentArray: Array<Accomplishment>) => {
    var content = accomplishmentArray.map((accomplishment, index) => (
  ...
  const renderDuties = (dutiesArray: Array<Duty>) => {
    var content = dutiesArray.map((duty, index) => (
      <Text style={[styles.subListStyle]} key={index}>
        - {duty.duty}
      </Text>
    ))
        ...
        <PDFViewer width={'100%'} height={760} showToolbar>
          <Document author="General Santos City Water District" subject="CS Form No. 212. Revised 2017" title="Personal Data Sheet">
            ...
            <Page size={[612.3, 935.4]} style={styles.page}>
              ...
              <Text
                style={{ fontFamily: 'ArialNarrowBoldItalic', fontSize: 11, padding: '13 0 0 13' }}
                render={({ pageNumber }) => {
                  if (pageNumber == 1) {
                    return `${''}`
                  } else {
                ...
                <View style={[styles.instructionsContainer]}>
                  <View style={[styles.rowContainer]}>
                    <View style={[styles.w15]}>
                      <Text style={[{ fontFamily: 'ArialBoldItalic', textAlign: 'center' }]}>Instructions:</Text>
                    </View>
                    <View style={[styles.w85]}>
                      ...
                      <Text style={[{ paddingTop: 15 }]}>
                        ...
                        full. For the current position, use the word Present, e.g., 1998-Present. Work experience should be listed from most
                        recent first.
                      </Text>
                    </View>
                  </View>
