...
export const College = (): JSX.Element => {
  ...
  const setDeletedColleges = usePdsStore((state) => state.setDeletedColleges)
  ...
  const onSubmit = handleSubmit((course: EducationInfo, e: any) => {
    ...
    const sortedUpdatedCollege = [...updatedCollege].sort((firstItem, secondItem) =>
      firstItem.from! > secondItem.from! ? -1 : secondItem.from! > firstItem.from! ? 1 : 0
    )
    setCollege(sortedUpdatedCollege)
    reset()
  ...
  const handleRemoveCourse = (courseIdx: number) => {
    ...
    const deleted = [...deletedColleges]

    deleted.push(removedCourse)

    // set the new value of deleted
    setDeletedColleges(deleted)
  ...
  return (
    <>
      <Card
        ...
        remarks={
          <>
            <Button btnLabel="Add College Course" type="button" variant="theme" onClick={openModal} shadow className="sm:w-full lg:w-56" />
          </>
        }
        children={
          <>
