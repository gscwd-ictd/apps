...
export const Vocational = (): JSX.Element => {
  ...
  const setDeletedVocationals = usePdsStore(
    (state) => state.setDeletedVocationals
  );
  ...
  const onSubmit = handleSubmit((course: EducationInfo, e: any) => {
    ...
    const sortedUpdatedVocational = [...updatedVocational].sort(
  ...
  const handleRemoveCourse = (courseIdx: number) => {
    ...
    const deleted = [...deletedVocationals];
    ...
    <>
      <Card
        ...
        subtitle={
          vocational.length === 0
            ? ''
            : 'Courses are sorted by the beginning year in descending order.'
        }
      >
        <>
          <Modal
