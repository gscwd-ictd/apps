...
interface MySelectBrgyProps extends React.DetailedHTMLProps<React.SelectHTMLAttributes<HTMLSelectElement>, HTMLSelectElement> {
  id: string
...
export const SelectBrgy: React.FC<MySelectBrgyProps> = ({
  ...
  useEffect(() => {
    ...
    if (!isEmpty(data)) {
      var filteredBrgys = data.data.filter((brgy: Brgy) => brgy.city_code === codeVariable.cityCode)

      var sortedBrgys = filteredBrgys.sort((firstItem: Brgy, secondItem: Brgy) =>
