...
export default function Inbox({ employeeDetails }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  useEffect(() => {
    if (swrIsLoadingMessages) {
      getPsbMessageList(swrIsLoadingMessages);
    }
  ...
  useEffect(() => {
    if (!isEmpty(swrMessages)) {
      getPsbMessageListSuccess(swrIsLoadingMessages, swrMessages);
    }

    if (!isEmpty(swrError)) {
      getPsbMessageListFail(swrIsLoadingMessages, swrError.message);
    }
