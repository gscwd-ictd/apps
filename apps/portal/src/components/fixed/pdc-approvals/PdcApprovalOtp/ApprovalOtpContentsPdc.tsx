...
interface OtpProps {
  ...
  tokenId: string; //like Training Id
  otpName: string;
  remarks?: string;
}

export const ApprovalOtpContentsPdc: FunctionComponent<OtpProps> = ({
  mobile,
  employeeId,
  action,
  tokenId,
  ...
  const handleClose = (e) => {
    e.preventDefault();
    setOtpPdcModalIsOpen(false); //close OTP modal first
    setTimeout(() => {
      setTrainingModalIsOpen(false); //then close Training modal
    }, 200);
  };
  ...
  // SUBMIT OTP CODE AND COMPLETE TRAINING APPROVAL/DISAPPROVAL IF CORRECT
        ...
        <>
          <div className="flex flex-col p-8 gap-1 justify-center items-center text-sm">
            <div className="mb-2 text-center">
              {`To ${action === PdcApprovalAction.APPROVE ? 'approve' : 'disapprove'} this Training, click Send Code
