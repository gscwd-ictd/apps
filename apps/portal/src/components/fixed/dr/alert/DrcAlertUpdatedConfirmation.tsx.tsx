...
import { UpdatedDRCD, UpdatedDutyResponsibility } from 'apps/portal/src/types/dr.type';
...
export const DrcAlertUpdatedConfirmation = () => {
  ...
  const { addedDrcs, cancelDrcPage, postDrcs, postDrcsFail, postDrcsSuccess, selectedDrcType } = useUpdatedDrcStore(
    (state) => ({
  ...
  const onSubmitConfirm = async () => {
    ...
    if (action === Actions.CREATE) {
      const drcsForPosting = await UpdateUpdatedFinalDrcs(addedDrcs);
      postPosition();
