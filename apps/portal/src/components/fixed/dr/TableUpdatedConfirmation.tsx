...
import { DutyResponsibility, UpdatedDutyResponsibility } from '../../../types/dr.type';
import { Table, TableBody, TableHeader } from '../table/Table';

type TableConfirmationProps = {
  array: Array<UpdatedDutyResponsibility>;
};

export const TableUpdatedConfirmation = ({ array }: TableConfirmationProps) => {
  return (
    <>
      {array.map((dr: UpdatedDutyResponsibility, index: number) => {
        return (
          <div key={index} className="min-w-[50rem]">
            <div className="flex items-center w-full py-3 my-1 text-sm border bg-slate-200 border-slate-100">
              <div className="w-[5%] flex justify-center">
                <HiCheck size={20} fill="#7b42f5" />
              </div>
              <div className="w-[40%] ">
                <p className="w-full pr-1"> {dr.duty}</p>
              </div>
