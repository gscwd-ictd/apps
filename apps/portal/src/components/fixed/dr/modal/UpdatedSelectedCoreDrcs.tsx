...
export const UpdatedSelectedCoreDrcs = (): JSX.Element => {
  ...
  const handleRemove = (positionIndexToRemove: number) => {
    ...
    const updatedSelectedCoreDnrs = [...addedDrcs.core];
    ...
    updatedSelectedCoreDnrs.map((dr: UpdatedDutyResponsibility, index: number) => {
      dr.sequenceNo = index;
    });

    // set the new value of selected core drs
    setAddedDrcs({ ...addedDrcs, core: updatedSelectedCoreDnrs });
  };

  const handleEditToggle = (sequenceNo: number, onEdit: boolean) => {
    const tempSelectedDnrs = JSON.parse(JSON.stringify(addedDrcs.core));
    ...
    setAddedDrcs({ ...addedDrcs, core: tempUpdatedSelectedDnrs });
  };

  const onChangePercentage = (e: any, sequenceNo: number) => {
    const tempSelectedDnrs = [...addedDrcs.core];
    const tempUpdatedSelectedDnrs: Array<UpdatedDutyResponsibility> = [];
          ...
          addedDrcs.core.map((dr: UpdatedDutyResponsibility, index: number) => {
            return (
              <div key={index} className="grid grid-cols-12 col-span-12 gap-1">
                ...
                <div className="col-span-1 ">
                  <div className="flex justify-center h-[1.5rem] font-light text-gray-800">
                    {UseRenderBadgePill(dr.competency.code)}
                  </div>
                </div>
                <div className="col-span-1">
                  <label className="flex justify-center h-[1.5rem] font-light text-gray-800">
