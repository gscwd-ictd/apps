...
export const AllSelectionApplicantsList = () => {
    ...
    const applicantScores = useAppSelectionStore(state => state.applicantScores)
    const setSelectedApplicants = useAppSelectionStore((state) => state.setSelectedApplicants);
    const setApplicantList = useAppSelectionStore((state) => state.setApplicantList);
    const setPublicationDetails = useAppSelectionStore((state) => state.setPublicationDetails);
    const setApplicantScores = useAppSelectionStore((state) => state.setApplicantScores);
    const selectedPublicationId = useAppSelectionStore((state) => state.selectedPublicationId);
    ...
    useEffect(() => {
        if (data && applicantListIsLoaded === false) {
            ...
            let fetchedData: any = [...data.ranking];

            let applicants = fetchedData.map((applicant: any, index: number) => {
            ...
            let scores = applicants.map((applicant: any) => {
                let tempScoreArray: Array<PsbScores> = [];
                if (parseInt(applicant.psb_1) > 0) tempScoreArray.push({ id: 1, score: applicant.psb_1 });
                if (parseInt(applicant.psb_2) > 0) tempScoreArray.push({ id: 2, score: applicant.psb_2 });
                if (parseInt(applicant.psb_3) > 0) tempScoreArray.push({ id: 3, score: applicant.psb_3 });
                if (parseInt(applicant.psb_4) > 0) tempScoreArray.push({ id: 4, score: applicant.psb_4 });
                if (parseInt(applicant.psb_5) > 0) tempScoreArray.push({ id: 5, score: applicant.psb_5 });
                if (parseInt(applicant.psb_6) > 0) tempScoreArray.push({ id: 6, score: applicant.psb_6 });
                if (parseInt(applicant.psb_7) > 0) tempScoreArray.push({ id: 7, score: applicant.psb_7 });
                if (parseInt(applicant.psb_8) > 0) tempScoreArray.push({ id: 8, score: applicant.psb_8 });
        ...
        <div className="min-h-auto max-h-[24rem] overflow-y-auto bg-slate-100 py-5 rounded-md">
            ...
            {applicantList.length > 0 ? (
                <div className='px-5'>
                    ...
                    <ul>
                        {applicantList.map((applicant: ApplicantWithScores, index: number) => {
                            return (
                                <li
                                    key={index}
                                    onClick={() => onSelect(applicant.sequenceNo!)}
                                    className={`flex grid-cols-3 items-center border-b gap-4 ${applicant.state && 'bg-slate-300'} w-full hover:cursor-pointer border-b-gray-100 border-l-transparent py-5 transition-colors ease-in-out hover:border-l-indigo-500 hover:bg-indigo-100`}
                                >
                                    ...
                                    <div className="w-[70%] ">
                                        <div className="w-full flex">
                                            <div className="w-full flex justify-center items-center">
                                                <p className="font-normal">{isEmpty(applicant.psb_1) ? `0.00` : applicant.psb_1}</p>
                                            </div>
                                            <div className="w-full flex justify-center items-center">
                                                <p className="font-normal">{isEmpty(applicant.psb_2) ? `0.00` : applicant.psb_2}</p>
                                            </div>
                                            <div className="w-full flex justify-center items-center">
                                                <p className="font-normal">{isEmpty(applicant.psb_3) ? `0.00` : applicant.psb_3}</p>
                                            </div>
                                            <div className="w-full flex justify-center items-center">
                                                <p className="font-normal">{isEmpty(applicant.psb_4) ? `0.00` : applicant.psb_4}</p>
                                            </div>
                                            <div className="w-full flex justify-center items-center">
                                                <p className="font-normal">{isEmpty(applicant.psb_5) ? `0.00` : applicant.psb_5}</p>
                                            </div>
                                            <div className="w-full flex justify-center items-center">
                                                <p className="font-normal">{isEmpty(applicant.psb_6) ? `0.00` : applicant.psb_6}</p>
                                            </div>
                                            <div className="w-full flex justify-center items-center">
                                                <p className="font-normal">{isEmpty(applicant.psb_7) ? `0.00` : applicant.psb_7}</p>
                                            </div>
                                            <div className="w-full flex justify-center items-center">
                                                <p className="font-normal">{isEmpty(applicant.psb_8) ? `0.00` : applicant.psb_8}</p>
                                            </div>
                                            <div className="w-full flex justify-center items-center">
