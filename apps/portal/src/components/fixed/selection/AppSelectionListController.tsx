...
export const AppSelectionModalController = ({ page }: AppPlaceListControllerProps) => {

    const setPublicationList = useAppSelectionStore((state) => state.setPublicationList)

    const setFilteredPublicationList = useAppSelectionStore((state) => state.setFilteredPublicationList)
    ...
    if (!data) {
        return (<>
            <div className="w-full h-[90%]  static flex flex-col justify-items-center items-center place-items-center">
                <SpinnerDotted speed={70} thickness={70} className="w-full flex h-full transition-all " color="slateblue" size={100} />
            </div>
        </>)
    }
