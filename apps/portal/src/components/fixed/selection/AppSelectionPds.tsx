...
import { AppSelectionViewPds } from './AppSelectionViewPds';

type AppEndPdsProps = {
  applicantDetails: Pick<Applicant, 'applicantId' | 'applicantType'>;
};

export const AppSelectionPds = ({ applicantDetails }: AppEndPdsProps) => {
  ...
  const { pds, setPds } = useAppSelectionStore((state) => ({
    pds: state.pds,
    setPds: state.setPds,
  }));
