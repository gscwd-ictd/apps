...
export const AllSelectionPublicationList = () => {
  const filteredPublicationList = useAppSelectionStore((state) => state.filteredPublicationList);
            ...
            return (
              ...
              >
                <div className=" w-full py-2 px-5 ">
                  <h1 className="font-medium text-lg text-gray-600">{item.positionTitle}</h1>

                  <p className="text-sm text-black font-semibold">{item.itemNumber}</p>
                  ...
                  {item.postingStatus === PublicationPostingStatus.APPOINTING_AUTHORITY_SELECTION ? (
                    ...
                    <p className="text-indigo-500 text-sm">Updated at {dayjs(item.updatedAt).format('MMMM d, YYYY')}</p>
                  ) : null}
                </div>
              </li>
            );
